...
<?php
		...
		while (($file = readdir($dir_pointer))!==FALSE){
			if ($file != "." && $file != ".." && $file != "Index.php"){
				...
				if ($this->image_valid(mime_content_type($attch_path.$file) == TRUE)){
			...
			?>		
				<tr>
					...
					<td class="forum_message_attaches">
						<input type="button" name="postattachfile" id="postattachfile"  class="mainbutton"  value="Удалить вложение" onclick="reload_attaches('/controlpanel/deleteattachfile<?php echo '?newsid='.$newsid.'&filename='.$file;  ?>')" />		
						</td>				
